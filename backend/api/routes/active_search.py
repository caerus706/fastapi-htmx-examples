...
def index(request: Request):
    ...
    return templates.TemplateResponse("active_search/index.html", context)


@router.post("/search", response_class=HTMLResponse)
def search(request: Request, search: Annotated[str, Form()] = None):
    if not search:
        context = {'request': request, 'results': data}
    else:
        def hits(search):
            ...
            data,)
        context = {'request': request, 'results': hits(search.lower())}
    return templates.TemplateResponse("active_search/search_results.html", context)